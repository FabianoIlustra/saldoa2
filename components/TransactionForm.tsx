...
const TransactionForm: React.FC<TransactionFormProps> = ({ categories, users, accounts, currentUser, initialData, onAdd, onClose }) => {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border border-white/10 dark:border-slate-800">
        <div className="p-8 bg-indigo-600 text-white flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold">{initialData ? 'Editar Lançamento' : 'Novo Lançamento'}</h2>
            <p className="text-indigo-100 text-xs opacity-80">{initialData ? 'Atualize os detalhes da transação' : 'Preencha os detalhes da transação'}</p>
          </div>
